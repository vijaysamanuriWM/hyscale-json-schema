...
**Command Layer**:  This layer parses the commands entered by the user and dispatches the input to respective command service.

**Service Spec Layer**: This layer reads the service spec from the file system and dispatches it to the workflow controller.

**Workflow Controller**: This controller reads the servicespec and controls the entire workflow of the deployment. The workflow is not chain but is controlled by the controller. Every individual component would process its task and responds back to the controller. This controller would give the necessary input to each component. Reads the kubernetes cluster details from `<user.home>`/.kube/config and the registry details from `<user.home>`/.docker/config.json.
...
*   Dockerfiles are generated at `<user.home>`/.hyscale/hyscale/apps/`<app_name>`/`<service_name>`/generated-files/dockerfiles/
*   Assume (`<user.home>`/.hyscale/hyscale/apps/`<app_name>`/`<service_name>`/generated-files/dockerfiles/ ) as _Relative Docker Path_.
...
COPY configure.sh hyscale/init-scripts/	##Copies configure commands as configure.sh

COPY run.sh hyscale/init-scripts/		##Copies run commands as run.sh
...
1. When buildspec is specified by the user without any artifacts, configure, run commands & scripts i.e, if the user needs a plain stack image to be used as the final image for deployment.

**Build & Push are skipped when :**

2. When there is no dockerfile or buildspec present in the servicespec i.e, the image is already available in the registry.

#### Key Points:

*   Docker image is built using `docker build` command in the directory “image.dockerfile.path” or _Relative Docker Path_.
*   Build logs are written to `<user.home>/.hyscale/hyscale/apps/<app_name>/<service_name>/logs/build.log`
...
*   Push logs are written to `<user.home>/.hyscale/hyscale/apps/<app_name>/<service_name>/logs/push.log`

### Manifest Generator Module

![manifest-generator](images/manifest-generator.jpg)

Generates the kube manifest from the given service spec. Manifest generation is a combination of different plugins. Each plugin is responsible to read the fields of service spec known by it and then generate respective snippets of kubernetes manifests. When all these snippets are joined / merged together we get a complete set of kube manifests. 
...
*   SecretsDataHandler : Secrets data is handled when secrets from servicespec are specified as a map only , if they are specified as a list of secrets, tool expects the user to create kubernetes secret prior to the deployment with the name `"<app_name>-<service_name>"`.
